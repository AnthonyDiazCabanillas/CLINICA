...
namespace Dat.Sql.ClinicaAD.ComprobantesAD
{
    public class ComprobantesAD
    {
        ...
        private ComprobantesE LlenarEntidadV2(IDataReader dr, ComprobantesE pComprobantesE)
        {
            ComprobantesE oComprobantesE = new ComprobantesE();
            switch (pComprobantesE.Orden)
            {
                case 2:
                    oComprobantesE.FechaEmisionTex = Convert.ToString(dr["fechaemision"]);
                    oComprobantesE.CodComprobante = Convert.ToString(dr["codcomprobante"]);
                    oComprobantesE.IDSede= Convert.ToInt32(dr["idsede"]);
                    oComprobantesE.Sede= Convert.ToString(dr["sede"]);
                    oComprobantesE.Monto= Convert.ToDouble(dr["monto"]);
                    oComprobantesE.Estado= Convert.ToString(dr["estado"]);
                    oComprobantesE.DscEstado= Convert.ToString(dr["DscEstado"]);
                    break;
            ...
            try
            {
                using (SqlConnection cnn = new SqlConnection(VariablesGlobales.CnnClinica))
                {
                    using (SqlCommand cmd = new SqlCommand("Sp_Comprobantes_Consulta", cnn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@idbuscar", pComprobantesE.IDBuscar);
                        cmd.Parameters.AddWithValue("@buscar", pComprobantesE.Buscar);
                        cmd.Parameters.AddWithValue("@medico", pComprobantesE.Medico);
                        cmd.Parameters.AddWithValue("@ruc", pComprobantesE.Ruc);
                        cmd.Parameters.AddWithValue("@comprobante", pComprobantesE.Comprobante);
                        cmd.Parameters.AddWithValue("@fecha", pComprobantesE.Fecha);
                        cmd.Parameters.AddWithValue("@numerolineas", pComprobantesE.NumeroLineas);
                        cmd.Parameters.AddWithValue("@orden", pComprobantesE.Orden);
                        cnn.Open();
                        dr = cmd.ExecuteReader();
                        while (dr.Read())
                        {
                            oComprobantesE = LlenarEntidadV2(dr, pComprobantesE);
        ...
        public int Sp_ComprobanteManuales2_Insert(ref ComprobantesE pComprobantesE)
        {
            bool xResult = false;
            int xReturn;
            try
            {
                using (SqlConnection cnn = new SqlConnection(VariablesGlobales.CnnClinica))
                {
                    using (SqlCommand cmd = new SqlCommand("Sp_ComprobanteManuales2_Insert", cnn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        // Parametros del Store
                        ...
                        cmd.Parameters.AddWithValue("@fecha", pComprobantesE.FechaCancelacion.ToString("dd/MM/yyyy"));
                        cmd.Parameters.AddWithValue("@codcomprobante", pComprobantesE.CodComprobante);
                        ...
                        cmd.Parameters.AddWithValue("@flagdolares", pComprobantesE.Flagdolares);
                        cmd.Parameters.AddWithValue("@concepto", pComprobantesE.Concepto);
                        cmd.Parameters.AddWithValue("@tipocambio", pComprobantesE.Tipodecambio);
                        cmd.Parameters.AddWithValue("@tipoimpresion", pComprobantesE.Tipoimpresion);
                        cmd.Parameters.AddWithValue("@codempresa", pComprobantesE.CodEmpresa);
        ...
        public int Sp_ComprobanteManual_EFACT_Insert_Host(ref ComprobantesE pComprobantesE, string pPCHostname)
        {
            bool xResult = false;
            int xReturn;
            try
            {
                using (SqlConnection cnn = new SqlConnection(VariablesGlobales.CnnClinica))
                {
                    using (SqlCommand cmd = new SqlCommand("Sp_ComprobanteManual_EFACT_Insert_Host", cnn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        // Parametros del Store
                        cmd.Parameters.AddWithValue("@codliquidacion", pComprobantesE.CodLiquidacion);
                        cmd.Parameters.AddWithValue("@pflg_electronico", pComprobantesE.FlgElectronico == true ? "S" : "N");
                        ...
                        cmd.Parameters.AddWithValue("@codempresa", pComprobantesE.CodEmpresa);
                        cmd.Parameters.AddWithValue("@codterminal", pComprobantesE.CodTerminal);
                        cmd.Parameters.AddWithValue("@hostname", pPCHostname);
