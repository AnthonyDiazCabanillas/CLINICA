...
namespace Bus.AgendaClinica.Clinica
{
    public class SitedsWs
    {
        ...
        public string RutaWS_Siteds { get; set; } = "http://200.48.199.90/WSSITEDS/Sistema/";// "http://softvanperu.com:8000/SANNA_SITEDS/";

        private string xRuc { get; set; } = "";
        private string xIAFAS { get; set; } = "";
        private string xSunasa { get; set; } = "";

        List<ClinicaE> lstClinica = new List<ClinicaE>();
        ...
        public void AsignaIAFA(string pRuc, string pSunasa, string pIAFAS)
        {
            xSunasa = pSunasa;
            xRuc = pRuc;
            xIAFAS = pIAFAS;
        ...
        public class AsegCodRequestFull : AsegCodRequest
        {
            public AsegCodResponse oAsegCodResponse = new AsegCodResponse();

            public AsegCodRequestFull(AsegCodRequest pAsegCodRequest)
            {
                SUNASA = pAsegCodRequest.SUNASA;
                IAFAS = pAsegCodRequest.IAFAS;
                RUC = pAsegCodRequest.RUC;
                NombresAfiliado = pAsegCodRequest.NombresAfiliado;
                ApellidoPaternoAfiliado = pAsegCodRequest.ApellidoPaternoAfiliado;
                ApellidoMaternoAfiliado = pAsegCodRequest.ApellidoMaternoAfiliado;
                CodigoAfiliado = pAsegCodRequest.CodigoAfiliado;
                CodTipoDocumentoAfiliado = pAsegCodRequest.CodTipoDocumentoAfiliado;
                NumeroDocumentoAfiliado = pAsegCodRequest.NumeroDocumentoAfiliado;
                CodProducto = pAsegCodRequest.CodProducto;
                DesProducto = pAsegCodRequest.DesProducto;
                NumeroPlan = pAsegCodRequest.NumeroPlan;
                CodTipoDocumentoContratante = pAsegCodRequest.CodTipoDocumentoContratante;
                NumeroDocumentoContratante = pAsegCodRequest.NumeroDocumentoContratante;
                NombreContratante = pAsegCodRequest.NombreContratante;
                CodParentesco = pAsegCodRequest.CodParentesco;
                TipoCalificadorContratante = pAsegCodRequest.TipoCalificadorContratante;
                CodEspecialidad = pAsegCodRequest.CodEspecialidad;
            }
        ...
        {
            string stringJson = Json.MethodPost(Url + "ConsultaAsegNom"
                , new Json.Parameters("CodTipoDocumentoAfiliado", objRequest.CodTipoDocumentoAfiliado)
                , new Json.Parameters("NumeroDocumentoAfiliado", objRequest.NumeroDocumentoAfiliado)
                , new Json.Parameters("RUC", objRequest.RUC)
                , new Json.Parameters("SUNASA", objRequest.SUNASA)
                , new Json.Parameters("IAFAS", objRequest.IAFAS)
                , new Json.Parameters("NombresAfiliado", objRequest.NombresAfiliado)
                , new Json.Parameters("ApellidoPaternoAfiliado", objRequest.ApellidoPaternoAfiliado)
                , new Json.Parameters("ApellidoMaternoAfiliado", objRequest.ApellidoMaternoAfiliado)
                , new Json.Parameters("CodEspecialidad", objRequest.CodEspecialidad));
            return stringJson;
        }


        public static Task<string> ConsultaAsegNomAsync(string Url, AsegNombRequest objRequest)
        {
            return Task.Run(() =>
            {
                string stringJson = Json.MethodPost(Url + "ConsultaAsegNom"
        ...
        public AsegCodRequest mtdAsignarAseguradoCorrecto(List<AsegNombResponse> oListAsegNombResponse, string DocIdentidad, bool SoloActivos = true)
        {
            ...
            for (int j = 0, loopTo = oListAsegNombResponse.Count - 1; j <= loopTo; j++)
            {
                ...
                if (oAsegNomb.CodEstado == "1" & oAsegNomb.NumeroDocumentoAfiliado == DocIdentidad)
                {
                    oAsegCodRequest = fnAsegCod(oAsegNomb);
                }
                ...
                //    break;
                //}
                //else if (oAsegNomb.CodigoAfiliado == CodAsegurado & oAsegNomb.CodEstado == "1" & oAsegNomb.NumeroDocumentoAfiliado == DocIdentidad & oAsegNomb.CodProducto == TipProductoAseguradora & xIAFAS == "40005")
                //{
                //    oAsegCodRequest = fnAsegCod(oAsegNomb);
                //    break;
                //    //}
            ...
            return oAsegCodRequest;
        }

        public List<AsegCodRequest> RetornaIAFASporDni(ClinicaE pClinicaE, string tipo_documento, string nro_documento)
        {
            string stringJson;
            string[] listaIafa = new string[] { "20001", "20002", "40004", "20005", "40006", "20004", "20029", "40005" };
            List<AsegCodRequest> lstAsegCodRequest = new List<AsegCodRequest>();
            List<AsegCodResponse> lstAsegCodResponse = new List<AsegCodResponse>();

            if (pClinicaE == null) return lstAsegCodRequest;
            if (nro_documento == null) return lstAsegCodRequest;
            if (nro_documento == "") return lstAsegCodRequest;

            foreach (var iafa in listaIafa)
            {
                AsegNombRequest oAsegNombRequest = new AsegNombRequest(pClinicaE.RUCSunasa, pClinicaE.CodigoSunasa, iafa);
                oAsegNombRequest.CodTipoDocumentoAfiliado = tipo_documento;
                oAsegNombRequest.NumeroDocumentoAfiliado = nro_documento;
                ...
                if (stringJson != null)
                {
                    ...
                    AsegCodRequest oAsegCodRequest = mtdAsignarAseguradoCorrecto(oListAsegNombResponse, oAsegNombRequest.NumeroDocumentoAfiliado, true);
                    ...
                    if (stringJson != null)
                    {
                        ...
                        if (oAsegCodResponse != null)
                        {
                            lstAsegCodRequest.Add(oAsegCodRequest);
                            //return lstAsegCodRequest;
                            //break;
                        }
                    }
                }
        ...
        public List<AsegCodRequest> RetornaIAFASporDniParalelo(ClinicaE pClinicaE, string tipo_documento, string nro_documento)
        {
            List<AsegCodRequest> lstAsegCodRequestFinal = new List<AsegCodRequest>();
            try
            {
                string[] listaIafa = new string[] { "20001", "20002", "40004", "20005", "40006", "20004", "20029", "40005" };
                List<AsegCodRequest> lstAsegCodRequest = new List<AsegCodRequest>();
                ...
                foreach (var iafa in listaIafa)
                {
                    AsegCodRequest oAsegCodRequest = new AsegCodRequest(pClinicaE.CodigoSunasa, iafa, pClinicaE.RUCSunasa);
                    oAsegCodRequest.CodTipoDocumentoAfiliado = tipo_documento;
                    oAsegCodRequest.NumeroDocumentoAfiliado = nro_documento;
                    lstAsegCodRequest.Add(oAsegCodRequest);
                }

                ParallelLoopResult parallelLoopResult = Parallel.ForEach(lstAsegCodRequest, (oAsegCodRequestParall, loopState) =>
                {
                    new ParallelOptions
                    {
                    ...
                    AsegCodRequest oAsegCodRequestTask = DoTask(oAsegCodRequestParall);
                    if (oAsegCodRequestTask.NumeroDocumentoContratante != "")
                    {
                        lstAsegCodRequestFinal.Add(oAsegCodRequestTask);
                        loopState.Stop();
                    }
                });

                return lstAsegCodRequestFinal;
            }
            catch (Exception)
            {
        ...
        private AsegCodRequest DoTask(AsegCodRequest pAsegCodRequest)
        {
            AsegCodRequest oAsegCodRequest = new AsegCodRequest();
            try
            {
                string stringJson;
                AsegNombRequest oAsegNombRequest = new AsegNombRequest(pAsegCodRequest.RUC, pAsegCodRequest.SUNASA, pAsegCodRequest.IAFAS);
                oAsegNombRequest.CodTipoDocumentoAfiliado = pAsegCodRequest.CodTipoDocumentoAfiliado;
                oAsegNombRequest.NumeroDocumentoAfiliado = pAsegCodRequest.NumeroDocumentoAfiliado;

                stringJson = ConsultaAsegNom(RutaWS_Siteds, oAsegNombRequest);
                if (stringJson != null)
                {
                    List<AsegNombResponse> oListAsegNombResponse = (List<AsegNombResponse>)Newtonsoft.Json.JsonConvert.DeserializeObject(stringJson, typeof(List<AsegNombResponse>));
                    oAsegCodRequest = mtdAsignarAseguradoCorrecto(oListAsegNombResponse, oAsegNombRequest.NumeroDocumentoAfiliado, true);
                    oAsegCodRequest.RUC = oAsegNombRequest.RUC;
                    oAsegCodRequest.SUNASA = oAsegNombRequest.SUNASA;
                    oAsegCodRequest.IAFAS = oAsegNombRequest.IAFAS;
